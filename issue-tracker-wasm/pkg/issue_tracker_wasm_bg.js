 ...
 * @returns {any}
 */
export function add_issue(state_json, title) {
    ...
    const ret = wasm.add_issue(ptr0, len0, ptr1, len1);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function close_issue(state_json, issue_id) {
    const ptr0 = passStringToWasm0(state_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.close_issue(ptr0, len0, issue_id);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
